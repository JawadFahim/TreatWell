...
// Get the search query and type from the URL
...
$sql = "SELECT brand_name, dosage_form, manufacturer FROM medicine_brand WHERE $search_type LIKE :search_query LIMIT :limit OFFSET :offset";
...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM medicine_brand WHERE $search_type LIKE :search_query");
$stmt->bindValue(':search_query', '%' . $search_query . '%', PDO::PARAM_STR);
$stmt->execute();
...
<div class="pagination">
    ...
    for ($p = $start; $p <= $end; $p++): ?>
        <a href="?search_query=<?php echo urlencode($search_query); ?>&search_type=<?php echo urlencode($search_type); ?>&page=<?php echo $p; ?>" class="<?php echo $p == $page ? 'active' : ''; ?>"><?php echo $p; ?></a>
    <?php endfor; ?>
    <?php if($num_pages > $end): ?>
        <span class="dots">...</span>
        <a href="?search_query=<?php echo urlencode($search_query); ?>&search_type=<?php echo urlencode($search_type); ?>&page=<?php echo $num_pages; ?>"><?php echo $num_pages; ?></a>
    <?php endif; ?>
    <?php if($page < $num_pages): ?>
        <a href="?search_query=<?php echo urlencode($search_query); ?>&search_type=<?php echo urlencode($search_type); ?>&page=<?php echo $page + 1; ?>" class="next">Next</a>
    <?php endif; ?>
</div>
